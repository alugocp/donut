...
fn build_donut(mut f: File, l: u64) -> Donut {
    ...
    for i in 0..LIMIT {
        ...
        let mut r =
            (buffer[4] as f64) * (buffer[5] as f64) * (buffer[6] as f64) * (buffer[7] as f64);
        let rot =
            (buffer[8] as f64) * (buffer[9] as f64) * (buffer[10] as f64) * (buffer[11] as f64);
        ...
        donut.sprinkles.push(Sprinkle {
            color: Rgba {
                data: [red as u8, green as u8, blue as u8, 255],
            },
            x: ((r * a.cos()) + m) as u32,
            y: ((r * a.sin()) + m) as u32,
            r: rot,
